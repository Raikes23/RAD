...
def create_traffic_matrix(matrix, traffic):
    ...
    if traffic == None or len(traffic) != matrix_size:
        ...
        print("Traffic matrix not defined. Using full-mesh logical topology with one unit of traffic.")
        print("Traffic matrix:")
        print(a)
        return a.tolist()
    else:
        # make sure traffic is np matrix 
        traffic = np.array(traffic)



        print("Traffic matrix defined.")
        print("Traffic matrix:")
        print(traffic)
        return traffic
...
matrix = [
    ...
    [0, 0, 2093.0, 0, 1452.8, 0, 0, 0, 1128.27, 0, 0, 1972, 0, 0],     # Node 6: Houston
    ...
    [0, 0, 0, 0, 0, 1972, 0, 0, 0, 0, 0, 0, 385.84, 246.96],           # Node 12: College Park
...
factor = 0.95 
matrix = [[element * factor for element in row] for row in matrix]

#Remove link Colege Pk-Princeton from the matrix (physical topology)
x, y= 12 - 1, 14 - 1  # 12 and 14 are the nodes that represent College Park and Princeton, respectively

matrix = remove_link(matrix, x, y)
...
max_load_distance = 688 # 688 was the max load on a link for the uncapacitated routing
max_load_hops = 509 # 509 was the max load on a link for the uncapacitated routing

capacited_routing_flag = False
...
if distance_metric == "hops":
    #unweighted matrix
    matrix = np.where(matrix_np > 0, 1, 0).tolist()

if capacited_routing_flag:
    if distance_metric == "distance":
        lmax = max_load_distance
        uij = a * lmax
        MAX_LINK_CAP = uij
...
#print the shape of the traffic matrix
...
def display_link_load_barchart(load_matrix, sorting_order, distance_metric, a=0):
    import matplotlib.pyplot as plt

    links = []
    loads = []
    for row in range(len(load_matrix)):
        for collumn in range(len(load_matrix[row])):
            if matrix[row][collumn] != 0:
                links.append(f'{row+1}-{collumn+1}')
                loads.append(load_matrix[row][collumn])
    # make title more descriptive with sorting order and distance metric with capital first letter
    
    if a == 0:
        title = 'Link Load - ' + sorting_order.capitalize() + ' - ' + distance_metric.capitalize()
    else:
        title = 'Link Load - ' + sorting_order.capitalize() + ' - ' + distance_metric.capitalize() + ' - a=' + str(a)
    ...
    #save to file in output folder "output_stor"
    filename = title + '.png'
    plt.savefig(filename)



display_link_load_barchart(load_matrix, sorting_order, distance_metric, a)
...
print("sorting_order: ", sorting_order)

if capacited_routing_flag:
    print("a: ", a)
    print("lmax: ", lmax)
    print("uij: ", uij)
    print("MAX_LINK_CAP: ", MAX_LINK_CAP)
